...
rm(DEGgeneral)
GS <- GeneSetTest(ENSG.all.genes$entrezID[ENSG.all.genes$ensembl_gene_id %in% genes], allgenes=ENSG.all.genes$entrezID[ENSG.all.genes$ensembl_gene_id %in% bkgenes], MHC=MHC)
GS <- entrez2symbol(GS)
GS$logP <- -log10(GS$p)
