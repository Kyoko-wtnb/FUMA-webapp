...
if(defined $chrcol && defined $poscol && defined $rsIDcol && defined $altcol && defined $refcol){
	...
	# 	my $dbSNPall = "$dbSNPfile/dbSNP146.chr$chr.vcf.gz";
	# 	my $fin = IO::Zlib -> new($dbSNPall, 'br');
	# 	while(<$fin>){
	# 		next if(/^#/);
	# 		my @line = split(/\s/, $_);
	# 		if(exists $GWAS{$line[2]}{"p"}){
	# 			if(defined $refcol && defined $altcol){
	# 				if(($line[3]==$GWAS{$line[2]}{"ref"} && $line[4]==$GWAS{$line[2]}{"alt"}) || ($line[4]==$GWAS{$line[2]}{"ref"} && $line[3]==$GWAS{$line[2]}{"alt"})){
	# 					print SNP join("\t", ($line[0], $line[1], $GWAS{$line[2]}{"ref"}, $GWAS{$line[2]}{"alt"}, $line[2], $GWAS{$line[0]}{"p"}));
	# 					print SNP "\t", $GWAS{$line[2]}{"or"} if(defined $orcol);
	# 					print SNP "\t", $GWAS{$line[2]}{"be"} if(defined $becol);
	# 					print SNP "\t", $GWAS{$line[2]}{"se"} if(defined $secol);
	# 					print SNP "\t", $GWAS{$line[2]}{"N"} if(defined $Ncol);
	# 					print SNP "\n";
	# 				}
	# 			}elsif(defined $altcol){
	# 				if($line[3]==$GWAS{$line[2]}{"alt"} || $line[4]==$GWAS{$line[2]}{"ref"}){
	# 					print SNP join("\t", ($line[0], $line[1], $GWAS{$line[2]}{"ref"}, $GWAS{$line[2]}{"alt"}, $line[2], $GWAS{$line[0]}{"p"}));
	# 					print SNP "\t", $GWAS{$line[2]}{"or"} if(defined $orcol);
	# 					print SNP "\t", $GWAS{$line[2]}{"be"} if(defined $becol);
	# 					print SNP "\t", $GWAS{$line[2]}{"se"} if(defined $secol);
