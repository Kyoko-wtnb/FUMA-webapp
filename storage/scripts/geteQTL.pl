...
my @ts;
if($tsall eq "all"){
	my @temp = `ls /media/sf_SAMSUNG/GTEx/Tabix/*.txt.gz`;
	chomp @temp;
	foreach my $f (@temp){
		$f =~ /Tabix\/(.+)\.txt\.gz/;
		push @ts, "GTEx_".$1;
...
foreach my $s (keys %db){
	...
	if($s eq "GTEx"){
		foreach my $f (@files){
			my $file = "/media/sf_SAMSUNG/GTEx/Tabix/".$f;
			$f =~ /(.+)\.txt.gz/;
			my $ts = $1;
			my $f2 = $ts.".sig.txt.gz";
			my $file2 = "/media/sf_SAMSUNG/GTEx/TabixSig/".$f2;
