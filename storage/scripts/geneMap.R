...
if(nrow(geneTable)>0){
	...
	if(ciMap==1){
		...
		if(nrow(ci)>0){geneTable$ciMapts[geneTable$ciMap=="Yes"] <- sapply(geneTable$ensg[geneTable$ciMap=="Yes"], function(x){paste(unique(ci$`tissue/cell`[grepl(x,ci$genes)]), collapse=":")})}
	}
