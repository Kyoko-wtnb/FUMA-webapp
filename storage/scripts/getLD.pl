...
my @chr15 = `ls /media/sf_Documents/VU/Data/Chr15States/States/*.bed.gz`; #local #local
...
=cut
	...
	#}else{
	print "Start chr $chr: \n";
	my $ldfile = "/media/sf_SAMSUNG/1KG/Phase3/".$pop."/".$pop.".chr$chr.ld.gz"; #local #local
#webserver 	my $ldfile = "/data/1KG/Phase3/".$pop."/".$pop.".chr$chr.ld.gz";

	# my $maffile = "/media/sf_SAMSUNG/1KG/Phase3/EUR/EUR.chr$chr.frq.gz";
	my $maffile = "/media/sf_SAMSUNG/1KG/Phase3/".$pop."_annot/chr$chr.data.txt.gz"; #local #local
...
system "/home/kyoko/annovar/annotate_variation.pl -out $annovout -build hg19 $annovin ~/annovar/humandb/ -dbtype ensGene"; #local #local
